...
class PluginSandbox:
    ...
    def _apply_resource_limits(self) -> None:
        ...
        if not HAS_RESOURCE_MODULE or _resource_mod is None or sys.platform == "win32":
            logger.debug(
        ...
        try:
            ...
            _resource_mod.setrlimit(_resource_mod.RLIMIT_AS, (memory_bytes, memory_bytes))

            # CPU time limit
            cpu_seconds = int(self.limits.max_cpu_seconds)
            _resource_mod.setrlimit(_resource_mod.RLIMIT_CPU, (cpu_seconds, cpu_seconds))

            # File size limit
            file_bytes = self.limits.max_file_size_mb * 1024 * 1024
            _resource_mod.setrlimit(_resource_mod.RLIMIT_FSIZE, (file_bytes, file_bytes))
