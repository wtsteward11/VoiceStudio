...
from backend.services.model_preflight_subprocess import (
    CHATTERBOX_PROBE_FILE,
    CHATTERBOX_REPO_ID,
    subprocess_chatterbox_hf_hub,
    subprocess_tortoise_models_ready,
)
...
def ensure_chatterbox(auto_download: bool = True) -> dict[str, object]:
    ...
    local_path, downloaded, hf_err = subprocess_chatterbox_hf_hub(
        python_exe,
        auto_download,
    )
...
def ensure_tortoise(auto_download: bool = True) -> dict[str, object]:
    ...
    if not has_weights and auto_download:
        downloaded, warm_err = subprocess_tortoise_models_ready(
            python_exe,
            tortoise_models_dir,
            True,
        )
