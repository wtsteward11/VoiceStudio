...
namespace VoiceStudio.App
{
  public sealed partial class MainWindow : Window
  {
    ...
    public MainWindow()
    {
      ...
      // Safe mode skips all panel creation so individual panels can be diagnosed in isolation.
      if (App.IsSafeMode)
      {
        ErrorLogger.LogWarning("[MainWindow] SAFE MODE ACTIVE — all panel creation skipped.", "MainWindow");
        if (this.Content is Grid rootGrid)
        {
          var safeBanner = new InfoBar
          {
            Title = "Safe Mode Active",
            Message = "Panels and backend startup are disabled. Unset VOICESTUDIO_SAFE_MODE to resume normal operation.",
            Severity = InfoBarSeverity.Warning,
            IsOpen = true,
            IsClosable = false,
            Margin = new Thickness(8, 4, 8, 0)
          };
          Grid.SetRow(safeBanner, 1);
          rootGrid.Children.Add(safeBanner);
        }
        profiler.Checkpoint("Safe Mode Banner Inserted");
      }
      else if (!RestorePanelsFromLayout())
      {
