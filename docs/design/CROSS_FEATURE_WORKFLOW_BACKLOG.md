...
**Workflow / STATE lane:** **[Workflow Coherence Pass 08](WORKFLOW_COHERENCE_PASS_08_QUALITY_BENCHMARK_PROFILE_COMPARISON.md)** — **W8-C1 closed** (historical product closure, 2026-03-25): Quality Benchmark operational UI; seam **`VoiceStudio.App.Tests.ViewModels.QualityBenchmarkViewModelSeamTests`** **8** passed; Quick **`artifacts/verify/20260325_191036`** (**Quick ≠ seam**; **`commit_hash`** **`bcd6d4e52e0b2a7763f0baaa261e7cdac7f8a665`**). **Hermetic compile-closure hardening** (2026-03-26): **`fix(build)`** **`8ba6363f`** — repo-wide committed tree build + Quick; authoritative **`artifacts/verify/20260326_020644`**; **`latest_pointer.json`** **`commit_hash`** **`8ba6363f8ccab333b675fd462c902a172983f76e`** (does **not** rerun W8-C1 QB seam; see [BUILD_INTEGRITY report](../reports/build/BUILD_INTEGRITY_NON_HERMETIC_W8C1_2026-03-25.md)). **W8-C3** — **§8.5** freeze + **§8.6** unsigned; **no** `src/` until sign-off. **Next bounded rows:** **W8-C2** / **W8-C3** only with **§5 + §8** discipline. **[Workflow Coherence Pass 07](WORKFLOW_COHERENCE_PASS_07_TRAINING_DATASET_MODEL_PROFILE.md)** — **W7-C1 closed** (§8.2); **Workflow 7 paused after W7-C1** ([§8.4](WORKFLOW_COHERENCE_PASS_07_TRAINING_DATASET_MODEL_PROFILE.md#84-workflow-7--continuation--pause-governance)) — Training-cluster **`src/`** reopen only with **signed** [§5 / §8](WORKFLOW_COHERENCE_PASS_07_TRAINING_DATASET_MODEL_PROFILE.md#8-sign-off-and-execution-record). **[Product trust Pass 01](PRODUCT_TRUST_AND_RELEASE_HONESTY_PASS_01.md)** remains **paused after slice 4** (§8.9 Option 1). **P05-Persist-A4** §12-gated. **Pass 06** `src/` only with new §5 row; **do not** open Pass 06 slice 5 by inertia.
...
| **Current wiring status** | **Quality Benchmark:** operational **`QualityBenchmarkView`** bindings (profile, text, engine toggles, run, results, next-step) — [Pass 08](WORKFLOW_COHERENCE_PASS_08_QUALITY_BENCHMARK_PROFILE_COMPARISON.md) §8.3. **A/B** + **Profile Comparison:** still placeholder / shell vs VM (Pass 08 §3–§4). **Product trust slice 4** footnote remains; cluster still **partial** until W8-C2/C3. |
| **Pass 08 status** | **W8-C1 closed** (2026-03-25): §8.2 **Tyler**; Quick **`20260325_191036`**; pointer **`commit_hash`** **`bcd6d4e52e0b2a7763f0baaa261e7cdac7f8a665`**. **W8-C3:** **§8.5** freeze + **§8.6** sign-off ([Pass 08](WORKFLOW_COHERENCE_PASS_08_QUALITY_BENCHMARK_PROFILE_COMPARISON.md)); **`src/`** blocked until signed. **W8-C2:** TBD (new §5 + §8). |
| **Known gaps/friction** | **W8-C2/C3** (A/B, Profile Comparison shells); no shared profile selection with **`IContextManager`**; **Profile Comparison** hard-codes **`"xtts"`**; **AB Testing** ctor **`LoadProfilesAsync`** fire-and-forget vs **`InitializeAsync`** on other panels. |
| **Recommended next improvement** | Product signs next **Pass 08** row: **W8-C2** (A/B UI) or **W8-C3** (Profile Comparison UI + engine policy) — **not** both without §5 expansion. |
| **Proof/test opportunity** | Pass 08 §7 discipline: `dotnet build`; seam `FullyQualifiedName~VoiceStudio.App.Tests.ViewModels.QualityBenchmarkViewModelSeamTests` **8** passed; `verify.ps1 -Quick` cited **separately**. |
...
| 8 | Quality / benchmark / comparison | **Pass 08** — **W8-C1 closed** (2026-03-25); **W8-C2/C3** → new §5/§8 only. |
