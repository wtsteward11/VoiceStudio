...
Manifest declares **`venv_family`: `venv_advanced_tts`** (shared with Chatterbox). Preflight **must** probe imports in that family’s `python.exe`, not the FastAPI backend `.venv` — same posture as `ensure_chatterbox`.
...
1. `from openvoice.api import BaseSpeakerTTS, ToneColorConverter` succeeds in the **`venv_advanced_tts`** interpreter (subprocess probe), and  
...
- Engine: `app/core/engines/openvoice_engine.py`.
