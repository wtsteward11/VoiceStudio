...
| **Product / engineering** | **P05-Persist-A4** accepted: **Path A only** — after each successful **`UploadAudioFileAsync`** in [`LibraryView.xaml.cs`](../../src/VoiceStudio.App/Views/Panels/LibraryView.xaml.cs) external-file drag-drop loop, if **`IContextManager.ActiveProjectId`** set, **`LibraryDragDropToProjectPersistence`** → **`SaveAudioToProjectAsync`** once per file with library id + **`Path.GetFileName(file.Path)`**; failures **non-blocking**, log **`SaveDragDropToProject`**; **`Path B`** **OUT** of A4 v1; **no** `AssetAddedEvent` / Transcribe C2 / **`ImportFilesAsync`** changes in this slice. | **2026-03-27 — Tyler** |
