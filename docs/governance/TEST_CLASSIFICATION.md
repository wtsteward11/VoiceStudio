...
| **R** | Runtime | Live backend on a port and/or desktop subprocess; health and feature paths through real I/O. | `verify.ps1` UI stages (icon-launch, failure smokes); `tests/ci/test_golden_loop_smoke_real.py` (real engine + consent); optional `PROOF_GOLDEN_PATH_REAL_*.json`; `artifacts/verify/<ts>/runtime_proof.json` **schema v2** from `verify.ps1 -RuntimeProof` (`status`: PASS / FAIL / BLOCKED; `proof_grade`: R). **GAP-015 slice 3:** sibling `artifacts/verify/<ts>/slo_baselines.json` **schema v1** — recorded p50/p95 (and p99 when sample_count ≥ 100) for three canonical ASGI-timed workflows; `baseline_policy: advisory` (does not gate CI). |
