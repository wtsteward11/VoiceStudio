...
- **App state**: `AppStateStore` (immutable) tracks backend connectivity and global app flags.
  ...
  - **Bounded Slice 18D (2026-04-20) — Tortoise runtime parity closed:** ADR-052 `venv_tortoise` subprocess path; live proof on **dedicated** `http://127.0.0.1:8028` with shared `VOICESTUDIO_REAL_XTTS_HTTP_BASE`; `pytest -m real_tortoise` **2/2**; C# Tortoise `LiveBackend` **3/3**; `TortoiseSubprocessEngine._worker_environ` rewrites **`router.huggingface.co`** → **`https://huggingface.co`** for worker Hub fetches; default subprocess + `real_tortoise` timeouts **2400s** (CPU jobs exceed 900s); C# preflight probe **120s** / synthesis client **40 min**. Proof [`docs/reports/verification/PROOF_SLICE18_TORTOISE_AUDITION.md`](docs/reports/verification/PROOF_SLICE18_TORTOISE_AUDITION.md) §Slice 18D; matrix `tortoise` **PASS**.
