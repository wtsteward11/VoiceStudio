...
- **App state**: `AppStateStore` (immutable) tracks backend connectivity and global app flags.
  ...
  - **Transcription job contract (2026-04-30)**: `POST /api/transcribe/jobs` (`TranscriptionJobRequest` / `TranscriptionJobResponse`) — explicit `simulate` path uses `build_simulation_transcript()` in `transcription_service.py` and persists via `TranscriptionRepository.store_transcription`; real path wraps `transcribe_audio`; `MLPreflightError` + `ServicePreflightError` + `ServiceError` map to `mode`/`status`/`blocker`. Tests: `tests/unit/backend/api/routes/test_transcription_job.py`; dialogue E2E: `TestTranscriptionToDialogueWorkflow` in `test_dialogue.py` (transcribe router mounted on dialogue `TestClient` fixture).
  - **Transcriptions DB + library clip paths (2026-04-30)**: Migration **`v005_transcriptions_updated_at`** adds missing **`updated_at`** on **`transcriptions`** so `TranscriptionRepository.update_transcription` → `BaseRepository.update` no longer raises SQLite `no such column: updated_at` when `create_timeline_clips_from_transcript` persists segment `timeline_clip_id`s. **`dialogue_segment_workflow._resolve_clip_source_path`** falls back to **`resolve_audio_path`** after `AudioRegistry` so library **`upload_id`** audio resolves from `audio_uploads/`; **`timeline.export`** `fallback_project_audio_id` uses the same resolver after registry miss.
  ...
  - **GAP-069 bounded slice 2 — Backend Readiness Regression Guard (2026-04-11)**: `scripts/ci/check_startup_artifact.py` validates schema v2 startup artifact (hard fail vs advisory timing); `tests/unit/test_startup_artifact_checker.py` **10**; `scripts/ci/run_backend_smoke.py` (slice 3 hardens proof schema — see below); `scripts/run_verification.py` includes `startup_artifact_check` (argv list when `VOICESTUDIO_STARTUP_ARTIFACT_PATH` set — fixes Windows path quoting); committed sample `docs/reports/verification/samples/startup_decision_success_v2.json`; skip env `VOICESTUDIO_SKIP_STARTUP_ARTIFACT_CHECK`. Closure: `docs/reports/verification/VOICESTUDIO_BACKEND_READINESS_REGRESSION_GUARD_LANE_CLOSURE_2026-04-11.md`.
  - **GAP-069 bounded slice 3 — Backend smoke enforcement (2026-04-11)**: `scripts/ci/run_backend_smoke.py` writes preserved **`PROOF_BACKEND_SMOKE_*.json`** schema **v1** (PASS/FAIL/BLOCKED; `blocking_reason` / `failure_reason` / `environment_hints`); BLOCKED prerequisites exit **2** writes proof + `verify.ps1 -BackendSmoke` exits **0** (advisory); `scripts/run_verification.py` **`backend_smoke_freshness`** mirrors `runtime_proof_staleness` policy (**BLOCKED** never fails; `--enforce-backend-smoke` fails missing/stale/FAIL); `verify.ps1` **`-BackendSmoke`** / **`-EnforceBackendSmoke`**; `tests/unit/test_backend_smoke_script.py` **10**; samples `docs/reports/verification/samples/backend_smoke_pass_v1.json` + `backend_smoke_blocked_v1.json`. Closure: `docs/reports/verification/VOICESTUDIO_BACKEND_READINESS_SMOKE_ENFORCEMENT_LANE_CLOSURE_2026-04-11.md`.
  - **GAP-069 bounded slice 4 — Backend smoke proof freshness (2026-04-12)**: PASS age prefers JSON **`timestamp_utc`** over file **`st_mtime`** (`_try_parse_iso_timestamp_utc`); advisory output includes **`age_source`**; expanded **MISSING** message; **`verify.ps1`** **`-SkipSmoke`** + **Backend Smoke Auto-Probe** before Gate/Ledger on full (non-Quick) runs; `tests/unit/test_backend_smoke_freshness_v4.py` **6**. Closure: `docs/reports/verification/VOICESTUDIO_BACKEND_READINESS_SMOKE_FRESHNESS_LANE_CLOSURE_2026-04-12.md`.
  - **GAP-069 bounded slice 5 — Full-verify Python collection stability (2026-04-12)**: **`backend/api/routes/search.py`** — no import-time **`get_*_for_search()`** / store materialization; **`_load_search_storage()`** (thread **`Lock`**, double-checked) on first **`GET /api/search`** request; **`STORAGE_AVAILABLE`** defaults **False**; **`tests/unit/backend/api/routes/test_search.py`** — repo-root **`sys.path`** uses **6** parents (match `conftest.py`); **`TestSearchCollectionSafety`** + **`TestSearchLazyLoader`**; **10** tests. Closure: `docs/reports/verification/VOICESTUDIO_BACKEND_READINESS_FULL_VERIFY_COLLECTION_STABILITY_LANE_CLOSURE_2026-04-12.md`. Full **`verify.ps1`** may still fail later in Python unit stage on unrelated tests (documented in closure).
  - **GAP-069 bounded slice 7 — STS durable marking full-verify test (2026-04-12)**: **`tests/unit/backend/services/test_sts_durable_marking.py`** — after **GAP-059**, **`GET /api/audio/{id}/marking`** awaits **`get_trust_audit_service().record_marking_read`**. Marking tests must patch **`backend.services.trust_audit_service.get_trust_audit_service`** and use **`AsyncMock`** for **`record_marking_read`**; use a non-colliding **`audio_id`** (e.g. **`plain-no-transform`**, not **`plain`** used elsewhere with **`is_transformed=True`**). **9** pytest PASS. Closure: `docs/reports/verification/VOICESTUDIO_BACKEND_READINESS_STS_DURABLE_MARKING_FULL_VERIFY_LANE_CLOSURE_2026-04-12.md`. Full **`verify.ps1`** then failed on **`test_profile_store::test_get_track`** (see closure §6).
  - **GAP-069 bounded slice 8 — Profile store / TrackStore full-verify harness (2026-04-13)**: **`tests/unit/backend/services/test_profile_store.py::TestTrackStore`** must mirror **`tests/unit/backend/test_track_store.py`**: per-test **`close_database_adapter` → `reset_database_adapter_singleton` → `reset_track_store` → `run_migrations` → `get_database_adapter(dbp)` → `await db.connect()`**; teardown resets singleton + new event loop. Regressions: **`test_disconnected_adapter_save_raises`** (`Database not connected`), **`test_connected_store_round_trips`**. **`test_profile_store.py`** **23** PASS. Closure: `docs/reports/verification/VOICESTUDIO_BACKEND_READINESS_PROFILE_STORE_FULL_VERIFY_LANE_CLOSURE_2026-04-13.md`. Umbrella **GAP-069** remains **Open** until full end-to-end **`verify.ps1`** is green (closure §7).
  - **GAP-069 bounded slice 13 — Failure-Path Smoke (Closed, 2026-04-14):** Producer ran after **`MainWindow`** and **`InitializeComponent`** could throw before backend/`failure_smoke_summary.json`; fix: failure-port + runtime-missing smokes skip **`MainWindow`**, run **`StartBackendWithTracking()`** first, strip inherited **`VOICE_STUDIO_SMOKE_*`** + kill stray **`VoiceStudio.App`** in harnesses; runtime-missing harness restores/cleans **`startup_decision.json`** for **Gate/Ledger**. Proof: **`-OnlyStage "Failure-Path Smoke"`** **PASS** **`artifacts/verify/20260413_200926/`**; checkpoint **`artifacts/verify/20260413_200958/`**; **`-ResumeFrom "Python Unit Tests"`** **PASS** **`artifacts/verify/20260413_203523/`**. Closure: [`VOICESTUDIO_GAP069_FAILURE_PATH_SMOKE_LANE_CLOSURE_2026-04-14.md`](docs/reports/verification/VOICESTUDIO_GAP069_FAILURE_PATH_SMOKE_LANE_CLOSURE_2026-04-14.md). Regression: `tests/unit/test_failure_path_smoke_contract.py`. Umbrella **GAP-069** **Closed** for bounded readiness chain (continuous CI items remain).
  - **GAP-069 bounded slice 12 — UI Smoke timeout (Closed, 2026-04-13):** Prior **`20260413_143616`** **TIMED_OUT** superseded. **Remediation:** FlaUI `SmokeTests` bounded polling (no infinite `GetMainWindow`), no `Application.Attach` under vstest, Win32 `EnumWindows` fallback, `VOICE_STUDIO_FLAUI_AUTOMATION` skips first-run wizard (`App.xaml.cs`), strip inherited smoke env from child, kill stray `VoiceStudio.App` (single-instance), Debug **newest** exe path, `[DoNotParallelize]`, scoped `dotnet test` filter to `SmokeTests`, **`verify.ps1`** UI Smoke sets **`VOICESTUDIO_USE_REAL_UI_AUTOMATION=true`**. Proof: **`verify.ps1 -OnlyStage "UI Smoke Tests"`** **PASS** `artifacts/verify/20260413_182528/` (~182s); **`verify.ps1 -ResumeFrom "Python Unit Tests"`** **PASS** `artifacts/verify/20260413_182858/`; **`python scripts/run_verification.py`** PASS. TRX may show **Skipped** when test host cannot enumerate WinUI HWNDs (documented). Progress/row **Closed**: `docs/reports/verification/VOICESTUDIO_GAP069_UI_SMOKE_TIMEOUT_2026-04-13.md`, `docs/design/GOV_VOICESTUDIO_BACKEND_READINESS_UI_SMOKE_TIMEOUT_12_EXECUTION_ROW.md`. Superseded for umbrella status by **slice 13** closure (2026-04-14).
  - **GAP-069 bounded slice 11 — Contract Tests resume blocker (Closed, 2026-04-13):** Initial resume **`20260413_124949`** had **Contract** **FAILED** (**17**); **fixed** — **`custom_openapi()`** stale **`_openapi_schema_generated`** vs OpenAPI cache clear caused static/incomplete spec (e.g. missing **`GET /api/voice/voices`**). Proof: **`pytest tests/contract/`** **238** passed / **5** skipped; **`verify.ps1 -OnlyStage "Contract Tests"`** **PASS**. Progress: `docs/reports/verification/VOICESTUDIO_GAP069_CONTRACT_TESTS_POST_RESUME_BLOCKER_2026-04-13.md`; execution row **Closed**: `docs/design/GOV_VOICESTUDIO_BACKEND_READINESS_CONTRACT_TESTS_RESUME_BLOCKER_11_EXECUTION_ROW.md`. Superseded for umbrella status by **slice 13** closure (2026-04-14).
  - **GAP-069 bounded slice 10 — pytest process-exit / harness stabilization (Closed, 2026-04-13):** **`EnhancedResourceManager`** unit tests call **`shutdown()`** (autouse fixture; **`type(obj) is _erm`** to avoid torch **`gc`** noise); session **`event_loop`** closes asyncgens + default executor; **`verify.ps1`** Python Unit Tests runs pytest via **`Start-Process`** with Win32 argument string, optional **TF_CPP_MIN_LOG_LEVEL** / **TRANSFORMERS_VERBOSITY**, polls **junitxml**, **120s** grace after green junit then **`Stop-Process`** if needed; **`Invoke-Stage`** exit-file read null-safe + **`cmd /c exit /b`** for timed stages. Proof: **`verify.ps1 -OnlyStage "Python Unit Tests"`** **PASSED** `artifacts/verify/20260413_120159/` (**5432** ~**284s**, ~**406s** wall with harness note). Closure: `docs/reports/verification/VOICESTUDIO_GAP069_PYTEST_PROCESS_EXIT_STABILITY_LANE_CLOSURE_2026-04-13.md`; execution row: `docs/design/GOV_VOICESTUDIO_BACKEND_READINESS_PYTEST_PROCESS_EXIT_STABILITY_10_EXECUTION_ROW.md`. Superseded for umbrella status by **slice 13** closure (2026-04-14); full CI/GHA **`verify.ps1`** remains continuous work.
  - **GAP-069 bounded slice 9 — verify.ps1 harness resume + timeout (Closed, 2026-04-13):** Monolithic full-certify plan invalidated (interactive run timed out during Python Unit Tests; no **`summary.json`**). Harness: **`checkpoint.json`** + incremental **`summary.json`** (`is_partial`), **`-ResumeFrom`**, **`-StopAfterStage`**, **Python Unit Tests** timeout **1200s**; **`stages`** JSON always an array. **Full AC6 proof:** **`-StopAfterStage "C# Unit Tests - Other"`** → 16 stages PASSED (`artifacts/verify/20260413_103636/`); **`-ResumeFrom "Python Unit Tests"`** → 16 INHERITED + TIMED_OUT (`artifacts/verify/20260413_104217/`) — **5432** tests passed in **261s** but process hung during teardown (TF/HF + VRAM alert loop); Quick **`artifacts/verify/20260413_110316/`** PASS. Closure: `docs/reports/verification/VOICESTUDIO_GAP069_VERIFY_HARNESS_RESUME_TIMEOUT_LANE_CLOSURE_2026-04-13.md`; execution row **Closed**: `docs/design/GOV_VOICESTUDIO_BACKEND_READINESS_VERIFY_HARNESS_RESUME_TIMEOUT_09_EXECUTION_ROW.md`. Superseded for Python stage exit by **slice 10** (junit + subprocess termination).
  - **GAP-069 bounded slice 6 — Transcribe regenerate full-verify test (2026-04-12)**: **`tests/unit/backend/api/routes/test_transcribe_regenerate.py`** — mock **`create_job`** at **`backend.services.canonical_job_lifecycle.create_job`** (route uses function-body `from backend.services.canonical_job_lifecycle import create_job` in `transcribe.py`; patching **`backend.api.routes.jobs.create_job`** was a stale contract). **7** pytest PASS. Closure: `docs/reports/verification/VOICESTUDIO_BACKEND_READINESS_TRANSCRIBE_REGENERATE_FULL_VERIFY_LANE_CLOSURE_2026-04-12.md`. Full **`verify.ps1`** then failed on unrelated **`test_sts_durable_marking`** (see slice 7).
  - **GAP-067 bounded slice 3 — Taskbar progress (2026-04-12)**: Unpackaged Win32 **`ITaskbarList3`** via `TaskbarProgressInterop` + `TaskbarProgressService` (`SetNormal` / indeterminate / error / clear); **`ShellProgressCoordinator`** implements **`IShellProgressPublisher`** with first-wins arbitration + pending queue; sources **`TranscribeViewModel`** (transcript apply correlation id) + **`TimelineViewModel`** (`timeline-synthesis`); **`MainWindow`** Loaded `WireTaskbarProgressShell` + Cleanup dispose; tests `Gap067Slice3Tests` **12** + `ShellProgressCoordinatorSeamTests` **6**. Closure: `docs/reports/verification/VOICESTUDIO_GAP067_TASKBAR_PROGRESS_LANE_CLOSURE_2026-04-12.md`.
  - **GAP-067 bounded slice 4 — File activation (2026-04-12)**: **`FileActivation`** / **`FileActivationArgs`** parse bare shell `argv` for **`.voiceproj`**, **`.vstudio`**, **`.vprofile`**; **`JumpListActivation.HasPending()`** gates so jump list wins; **`App.xaml.cs`** `FileActivation.SetPendingIfParsed` when no jump-list pending; **`MainWindow`** `TryDispatchPendingFileActivation` after jump list, startup **`IsReady`** deferral; **`IProjectWorkflowCoordinator.OpenProjectByPathAsync`** → **`TimelineProjectHandlers`** → **`IProjectRepository.OpenProjectFileAsync`**; **`.vstudio`**/**`.vprofile`** honest degradation (toast + picker / Profiles nav); Inno + WiX **`.vstudio`** HKCR **`VoiceStudio.Collaboration`**; tests `Gap067Slice4Tests` **14** + `FileActivationSeamTests` **6**. Closure: `docs/reports/verification/VOICESTUDIO_GAP067_FILE_ACTIVATION_LANE_CLOSURE_2026-04-12.md`.
  - **GAP-067 bounded slice 5 — Progressive disclosure (2026-04-12)**: **`MainWindow_StatusBar_SystemMetricsButton`** flyout for CPU/GPU/RAM/format/latency/collaborators; **`CustomizableToolbar_PerformanceOverflowButton`** flyout for dynamic perf toolbar items; **`VoiceSynthesisView_AdvancedControlsExpander`** (persisted **`VoiceSynthesis_AdvancedControlsExpanded`**); **`TimelineView_TransportMoreButton`** flyout (record/loop/zoom); disabled track **M/S/R** row **`Visibility=Collapsed`**; **`TranscribeView_AdvancedOptionsExpander`** (project id + STT toggles). Tests: `Gap067Slice5Tests` **7** + seam **3**. Closure: `docs/reports/verification/VOICESTUDIO_GAP067_PROGRESSIVE_DISCLOSURE_LANE_CLOSURE_2026-04-12.md`.
  - **GAP-067 bounded slice 6 — WCAG 2.1 AA (2026-04-12)**: Accessible naming on Slice 5 surfaces (`AutomationProperties.Name`, `LabeledBy`, 7 AutomationIds). `Gap067Slice6Tests` **8**. Closure: `docs/reports/verification/VOICESTUDIO_GAP067_WCAG_LANE_CLOSURE_2026-04-12.md`.
  - **GAP-067 bounded slice 7 — Cold-start (2026-04-12)**: `ColdStartTimingCollector` → `%LOCALAPPDATA%\VoiceStudio\Logs\cold_start_timing.json` (T1/T2 + markers); lazy **`RecentProjectsService`** (`EnsureRecentDataLoaded`); **`JumpListService.ScheduleInitialRebuildAfterDelay`**; **`StatusBarCoordinator.StartBackendHealthMonitoring`** from Loaded; deferred init records + artifact write after T2 + deferred complete; `Gap067Slice7Tests` **5**. Closure: `docs/reports/verification/VOICESTUDIO_GAP067_COLD_START_LANE_CLOSURE_2026-04-12.md`. Umbrella **GAP-067** **Closed** (all 7 slices).
  - **GAP-067 bounded slice 2 — Jump lists (2026-04-12)**: Unpackaged app uses Win32 **`ICustomDestinationList`** (`JumpListInterop.cs`, `JumpListService.cs`) — not MSIX `Windows.UI.StartScreen.JumpList`. **`JumpListService`** singleton projects **`RecentProjectsService.AllProjects`** (cap 10, debounced); static tasks `--jumplist-new` / `--jumplist-open-dialog`; recents `--jumplist-open "{path}"`; **`JumpListActivation`** + **`MainWindow`** pending dispatch after **`IStartupStateService.IsReady`**; tests `Gap067Slice2Tests` **11** + `JumpListServiceSeamTests` **6**. **`TranscribeViewModel`** apply-job progress reporter ignores stale Queued/Running after terminal Succeeded/Failed (full-suite flake fix). Closure: `docs/reports/verification/VOICESTUDIO_GAP067_JUMP_LISTS_LANE_CLOSURE_2026-04-12.md`.
  - **GAP-067 bounded slice 1 — Notification center core (2026-04-12)**: Single authority remains **`INotificationCenterService`** / `NotificationCenterService` (`AppNotificationItem`); **`NotificationCenterViewModel`** binds list + `MarkAllReadCommand` / `DismissItemCommand`; **`MainWindow`** title bar bell + flyout + unread badge (Loaded wiring); **`ToastNotificationService`** still feeds the center; **`StatusBarCoordinator.OnDegradedModeChanged`** adds **Warning** / **High** “Backend Unavailable” on degraded entry; tests `Gap067Tests` **9** + `NotificationCenterViewModelSeamTests` **8**. Closure: `docs/reports/verification/VOICESTUDIO_GAP067_NOTIFICATION_CENTER_CORE_LANE_CLOSURE_2026-04-12.md`.
...
- **Bounded Slice 9 (2026-04-17) — Playback / artifact audition (XTTS only) — runtime closed**: Gate with `GET /api/health/preflight` (`checks.xtts_v2.ok`). `pytest -m real_xtts` **2 passed** (primary `GET /api/audio/file/{id}` + audible WAV). `PlaybackAuditionLiveBackendTests`: **`Synthesize_PrimaryFileRoute_LiveBackend_StreamPlayable`** (RIFF/fmt/peak, writes `slice9_csharp_stream.wav`, no audio device) + **`Synthesize_ThenPlayback_LiveBackend_PlayableNonSilentWav`** (NAudio completion). Artifacts: `docs/reports/verification/slice9/` (`slice9_output.wav`, `slice9_csharp_stream.wav`, log snippet). Proof `docs/reports/verification/PROOF_SLICE9_PLAYBACK_AUDITION.md`. Not a statement about all engines.
- **Bounded Slice 10 (2026-04-18) — Engine parity (Piper only) — Closed**: preflight `checks.piper.ok`; Python `real_piper` **2/2**; C# `RealSynthesisPiperLiveBackendTests` **1/1**; C# `PiperPlaybackAuditionLiveBackendTests` stream + NAudio **1/1** each; NAudio proof line recorded in `PROOF_SLICE10_PIPER_AUDITION.md` §3 (repo uvicorn `http://127.0.0.1:8030` — port `8020` may lack Piper registration). `routed_engine=="piper"` asserted; `_get_piper_voice_v1` root-cause fix. Matrix `docs/reports/verification/ENGINE_PARITY_MATRIX.md`. Proof `docs/reports/verification/PROOF_SLICE10_PIPER_AUDITION.md`. Not a statement about non-Piper engines.
- **Bounded Slice 11 (2026-04-18) — No automatic utility TTS fallback — Closed**: Removed `_try_utility_tts_fallback` from `synthesis_service.py` and `voice/_helpers.py` (Option B; explicit `synthesize_with_utility` + deps unchanged). Proof `docs/reports/verification/PROOF_SLICE11_NO_FALLBACKS_REMOVAL.md`; discovery `docs/reports/verification/PROOF_SLICE11_FALLBACK_REMOVAL_DISCOVERY.md`.
...
## WCAG 2.1 AA Accessibility Patterns (2026-04-12)

- **AutomationProperties.Name**: All icon-only buttons require `AutomationProperties.Name` with a short English phrase (verb phrases for actions: "Play", "Stop", "System metrics"; noun phrases for display: "Performance metrics"). Screen readers cannot announce icon-only buttons without this.
- **LabeledBy binding**: Sliders and other controls without inherent labels use `AutomationProperties.LabeledBy="{x:Bind LabelName}"` where the label `TextBlock` has `x:Name="LabelName"`. Proven pattern: `SpectrogramView.xaml` (8 instances), `VoiceSynthesisView.xaml` (5 instances).
- **Expander accessible naming**: WinUI Expander shows header text visually but requires explicit `AutomationProperties.Name` for programmatic accessibility. Set to match the header text content.
- **Flyout keyboard**: WinUI Flyout provides built-in keyboard support -- Tab+Space/Enter to open, Escape to dismiss, focus returns to trigger. No custom handling needed.
- **WinUI collapsed elements**: Controls with `Visibility="Collapsed"` are automatically removed from the automation tree -- no manual tab order management required.
- **AutomationId convention**: Pattern is `{ViewName}_{ControlPurpose}` (e.g., `TimelineView_PlayButton`, `MainWindow_StatusBar_CollaboratorsButton`). All new IDs registered in `docs/developer/AUTOMATION_ID_REGISTRY.md`.
- **Contract test pattern**: Source-contract tests in `Gap067Slice6Tests.cs` assert string presence in XAML files (e.g., `StringAssert.Contains(text, "AutomationProperties.Name=\"System metrics\"")`) using `FindRepoRoot()` + `ReadAppFile()` helper pattern from Slice 5.
