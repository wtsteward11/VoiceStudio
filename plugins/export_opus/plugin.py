...
class OpusExporterPlugin(Plugin):
    """Opus format exporter plugin.

    Uses :class:`AudioConversionService` to convert WAV input to Opus.
    Validates output paths against the configured export root.
    ...
    def __init__(self, plugin_dir: Path):
        ...
        self.router = APIRouter(prefix="/api/plugin/export_opus", tags=["export_opus"])
    ...
    async def export(self, audio_data: bytes, output_path: Path, options: dict[str, Any]) -> bool:
        """Export audio data to Opus format."""
...
def register(app, plugin_dir: Path):
    ...
    try:
        plugin = OpusExporterPlugin(plugin_dir)
        plugin.register(app)
        plugin.initialize()
        return plugin
