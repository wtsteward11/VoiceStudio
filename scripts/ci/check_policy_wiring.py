#!/usr/bin/env python3
"""
CI check: verify trust & safety hooks are called from voice.py.
...
CHECKS = [
    (
        ...
        "Policy enforcer must be in voice router deps",
    ),
    (
        "backend/api/routes/voice.py",
        "write_provenance_sidecar",
        "Provenance must be called on synthesis success",
    ),
    (
        "backend/api/routes/voice.py",
        "record_synthesis_minutes",
        "Usage stats must be called on synthesis success",
    ),
    (
