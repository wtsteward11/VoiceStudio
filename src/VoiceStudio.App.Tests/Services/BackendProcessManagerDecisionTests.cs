...
public class BackendProcessManagerDecisionTests
{
    ...
    public async Task EnsureBackendRunningAsync_WhenHealthyBackendExists_WritesReuseDecision()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(3, root.GetProperty("schema_version").GetInt32());
    ...
    public async Task EnsureBackendRunningAsync_WhenBackendMissing_WritesSpawnDecision()
    {
        ...
        try
        {
            ...
            // Slice 5: baseline_deps_valid must be present in spawn decision
            Assert.IsTrue(root.TryGetProperty("baseline_deps_valid", out var bdv));
            Assert.AreEqual(true, bdv.GetBoolean());
        }
        finally
        {
            if (File.Exists(repoVenvPythonBackup))
    ...
    [TestMethod]
    public void DebugBuild_InterpreterPriority_DotVenvBeforeVenv()
    {
        // Verify that in Debug builds, .venv is preferred over venv
        // by inspecting the candidate array order. We construct the same
        // candidates array that BackendProcessManager uses.
...
#endif
        var dotVenvIdx = Array.FindIndex(pythonCandidates, p => p.Contains(".venv"));
        var venvIdx = Array.FindIndex(pythonCandidates, p => p.Contains("venv") && !p.Contains(".venv"));

#if DEBUG
        Assert.IsTrue(
            dotVenvIdx < venvIdx,
            $"In Debug build, .venv (index {dotVenvIdx}) must precede venv (index {venvIdx}) in candidate list.");
#else
        Assert.IsTrue(
            venvIdx < dotVenvIdx,
            $"In Release build, venv (index {venvIdx}) must precede .venv (index {dotVenvIdx}) in candidate list.");
#endif
    }
}
