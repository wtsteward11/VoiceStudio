...
{
  /// <summary>
  /// Slice 17 Chatterbox TTS — Live-backend proof: Chatterbox TTS synthesis → <see cref="IVoiceSynthesisService.GetAudioStreamAsync"/> →
  ...
  /// <see cref="RealSynthesisChatterboxLiveBackendTests"/> (<c>VOICESTUDIO_REAL_XTTS_HTTP_BASE</c>).
  /// </summary>
  [TestClass]
  [TestCategory("LiveBackend")]
  public sealed class ChatterboxPlaybackAuditionLiveBackendTests
  {
    ...
    /// <summary>
    /// Slice 17 Chatterbox TTS stream proof: Chatterbox TTS → <see cref="IVoiceSynthesisService.GetAudioStreamAsync"/> → non-silent WAV
    /// (RIFF/fmt/data, duration, peak; PCM16 or IEEE float32 per WAV spec) and writes <c>docs/reports/verification/slice17/chatterbox/chatterbox_csharp_stream.wav</c>.
    /// No audio output device required. On Chatterbox TTS unavailable, <see cref="Assert.Fail"/> (preflight should gate).
    /// </summary>
    [TestMethod]
    public async Task Synthesize_PrimaryFileRoute_LiveBackend_StreamPlayable()
    {
      ...
      {
        Assert.Inconclusive(
          "Set VOICESTUDIO_TEST_MODE unset (not stub) on the backend process for real Chatterbox TTS proof.");
      }
      ...
      try
      {
        profile = await profilesClient.CreateProfileAsync(
          "csharp-slice17-chatterbox-stream-playable",
          language: "en",
      ...
      using (var consentHttp = new HttpClient { BaseAddress = new Uri(BackendBase), Timeout = TimeSpan.FromMinutes(2) })
      {
        var consentReq = JsonSerializer.Serialize(
          new Dictionary<string, string>
          {
            ["voice_id"] = profile.Id,
            ["grantor_id"] = "local",
            ["grantor_name"] = "csharp-slice17-chatterbox-stream",
      ...
      try
      {
        response = await synthService.SynthesizeVoiceAsync(
          new VoiceSynthesisRequest
          {
            ...
            Engine = "chatterbox",
            Text = "VoiceStudio slice seventeen chatterbox playback artifact audition proof.",
            Language = "en",
          },
      ...
      {
        Assert.Fail(
          "Live Chatterbox TTS engine not initialized or unavailable; run /api/health/preflight with chatterbox.ok before this proof: "
          + ex.Message);
      ...
      var outPath = Path.Combine(
        ...
        "slice17",
        "chatterbox",
        "chatterbox_csharp_stream.wav");
      Directory.CreateDirectory(Path.GetDirectoryName(outPath)!);
      await File.WriteAllBytesAsync(outPath, bytes, CancellationToken.None).ConfigureAwait(false);
      Assert.IsTrue(File.Exists(outPath) && new FileInfo(outPath).Length == bytes.Length, "Failed to write slice17/chatterbox_csharp_stream.wav.");
    }
    ...
    public async Task Synthesize_ThenPlayback_LiveBackend_PlayableNonSilentWav()
    {
      ...
      {
        profile = await profilesClient.CreateProfileAsync(
          "csharp-slice17-chatterbox-playback-audition",
          language: "en",
          cancellationToken: CancellationToken.None).ConfigureAwait(false);
      }
      catch (Exception ex)
      ...
      using (var consentHttp = new HttpClient { BaseAddress = new Uri(BackendBase), Timeout = TimeSpan.FromMinutes(2) })
      {
        var consentReq = JsonSerializer.Serialize(
          new Dictionary<string, string>
          {
            ["voice_id"] = profile.Id,
            ["grantor_id"] = "local",
            ["grantor_name"] = "csharp-slice17-chatterbox-playback",
            ["consent_type"] = "voice_usage",
          });
        using var reqContent = new StringContent(
          consentReq,
      ...
      var tempPath = Path.Combine(Path.GetTempPath(), $"vs_slice17_chatterbox_{Guid.NewGuid():N}.wav");
