...
{
  /// <summary>
  /// Slice 12 eSpeak NG — Live-backend proof: eSpeak NG synthesis → <see cref="IVoiceSynthesisService.GetAudioStreamAsync"/> →
  ...
  /// <see cref="RealSynthesisESpeakNgLiveBackendTests"/> (<c>VOICESTUDIO_REAL_XTTS_HTTP_BASE</c>).
  /// </summary>
  [TestClass]
  [TestCategory("LiveBackend")]
  public sealed class ESpeakNgPlaybackAuditionLiveBackendTests
  {
    ...
    /// <summary>
    /// Slice 12 eSpeak NG stream proof: eSpeak NG → <see cref="IVoiceSynthesisService.GetAudioStreamAsync"/> → non-silent PCM16 WAV
    /// (RIFF/fmt/data, duration, peak) and writes <c>docs/reports/verification/slice12/espeak_ng/espeak_ng_csharp_stream.wav</c>.
    /// No audio output device required. On eSpeak NG unavailable, <see cref="Assert.Fail"/> (preflight should gate).
    /// </summary>
    [TestMethod]
    public async Task Synthesize_PrimaryFileRoute_LiveBackend_StreamPlayable()
    {
      ...
      {
        Assert.Inconclusive(
          "Set VOICESTUDIO_TEST_MODE unset (not stub) on the backend process for real eSpeak NG proof.");
      }
      ...
      await LivePreflightGuards.AssertEspeakNgPreflightOkAsync(probe, BackendBase, CancellationToken.None)
      ...
      try
      {
        profile = await profilesClient.CreateProfileAsync(
          "csharp-slice12-espeak-ng-stream-playable",
          language: "en",
      ...
      using (var consentHttp = new HttpClient { BaseAddress = new Uri(BackendBase), Timeout = TimeSpan.FromMinutes(2) })
      {
        var consentReq = JsonSerializer.Serialize(
          new Dictionary<string, string>
          {
            ["voice_id"] = profile.Id,
            ["grantor_id"] = "local",
            ["grantor_name"] = "csharp-slice12-espeak-ng-stream",
      ...
      try
      {
        response = await synthService.SynthesizeVoiceAsync(
          new VoiceSynthesisRequest
          {
            ...
            Engine = "espeak_ng",
            Text = "VoiceStudio slice twelve espeak ng playback artifact audition proof.",
            Language = "en",
          },
      ...
      catch (BackendException ex) when (ex.StatusCode == 400)
      {
        ...
        if (m.Contains("Invalid engine", StringComparison.OrdinalIgnoreCase)
            && m.Contains("espeak_ng", StringComparison.OrdinalIgnoreCase))
        {
          Assert.Fail(
            "Synthesis returned 400 Invalid engine after checks.espeak_ng preflight ok; "
      ...
      catch (BackendException ex) when (LiveEngineBackendTestGuards.IsLiveEngineUnavailable(ex, "espeak_ng"))
      {
        Assert.Fail(
          "Live eSpeak NG engine not initialized or unavailable; run /api/health/preflight with espeak_ng.ok before this proof: "
          + ex.Message);
      ...
      Assert.AreEqual(
        ...
        "Backend must echo routed_engine=espeak_ng (no silent engine substitution).");
      ...
      var outPath = Path.Combine(
        ...
        "slice12",
        "espeak_ng",
        "espeak_ng_csharp_stream.wav");
      Directory.CreateDirectory(Path.GetDirectoryName(outPath)!);
      await File.WriteAllBytesAsync(outPath, bytes, CancellationToken.None).ConfigureAwait(false);
      Assert.IsTrue(File.Exists(outPath) && new FileInfo(outPath).Length == bytes.Length, "Failed to write slice12/espeak_ng_csharp_stream.wav.");
    }
    ...
    public async Task Synthesize_ThenPlayback_LiveBackend_PlayableNonSilentWav()
    {
      ...
      {
        profile = await profilesClient.CreateProfileAsync(
          "csharp-slice12-espeak-ng-playback-audition",
          language: "en",
          cancellationToken: CancellationToken.None).ConfigureAwait(false);
      }
      catch (Exception ex)
      ...
      using (var consentHttp = new HttpClient { BaseAddress = new Uri(BackendBase), Timeout = TimeSpan.FromMinutes(2) })
      {
        var consentReq = JsonSerializer.Serialize(
          new Dictionary<string, string>
          {
            ["voice_id"] = profile.Id,
            ["grantor_id"] = "local",
            ["grantor_name"] = "csharp-slice12-espeak-ng-playback",
            ["consent_type"] = "voice_usage",
          });
        using var reqContent = new StringContent(
          consentReq,
      ...
      var tempPath = Path.Combine(Path.GetTempPath(), $"vs_slice12_espeak_ng_{Guid.NewGuid():N}.wav");
