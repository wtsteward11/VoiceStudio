...
{
  /// <summary>
  /// Slice 19A OpenVoice — Live-backend proof: OpenVoice synthesis → <see cref="IVoiceSynthesisService.GetAudioStreamAsync"/> →
  ...
  /// <see cref="RealSynthesisOpenVoiceLiveBackendTests"/> (<c>VOICESTUDIO_REAL_XTTS_HTTP_BASE</c>).
  /// </summary>
  [TestClass]
  [TestCategory("LiveBackend")]
  public sealed class OpenVoicePlaybackAuditionLiveBackendTests
  {
    ...
    /// <summary>
    /// Slice 19A OpenVoice stream proof: OpenVoice → <see cref="IVoiceSynthesisService.GetAudioStreamAsync"/> → non-silent WAV
    /// (RIFF/fmt/data, duration, peak; PCM16 or IEEE float32 per WAV spec) and writes <c>docs/reports/verification/slice19/openvoice/openvoice_csharp_stream.wav</c>.
    /// No audio output device required. On OpenVoice unavailable, <see cref="Assert.Fail"/> (preflight should gate).
    /// </summary>
    [TestMethod]
    public async Task Synthesize_PrimaryFileRoute_LiveBackend_StreamPlayable()
    {
      ...
      {
        Assert.Inconclusive(
          "Set VOICESTUDIO_TEST_MODE unset (not stub) on the backend process for real OpenVoice proof.");
      }
      ...
      await LivePreflightGuards.AssertOpenVoicePreflightOkAsync(probe, BackendBase, CancellationToken.None)
      ...
      try
      {
        profile = await profilesClient.CreateProfileAsync(
          "csharp-slice19-openvoice-stream-playable",
          language: "en",
      ...
      var fixtureWav = OpenVoiceProofFixtures.ResolveOpenVoiceReferenceWavPath(FindRepoRoot());
      ...
      using (var consentHttp = new HttpClient { BaseAddress = new Uri(BackendBase), Timeout = TimeSpan.FromMinutes(2) })
      {
        var consentReq = JsonSerializer.Serialize(
          new Dictionary<string, string>
          {
            ["voice_id"] = profile.Id,
            ["grantor_id"] = "local",
            ["grantor_name"] = "csharp-slice19-openvoice-stream",
      ...
      try
      {
        response = await synthService.SynthesizeVoiceAsync(
          new VoiceSynthesisRequest
          {
            ...
            Engine = "openvoice",
            Text = "VoiceStudio slice nineteen openvoice playback artifact audition proof.",
            Language = "en",
          },
      ...
      catch (BackendException ex) when (ex.StatusCode == 400)
      {
        ...
        if (m.Contains("Invalid engine", StringComparison.OrdinalIgnoreCase)
            && m.Contains("openvoice", StringComparison.OrdinalIgnoreCase))
        {
          Assert.Fail(
            "Synthesis returned 400 Invalid engine after checks.openvoice preflight ok; "
      ...
      catch (BackendException ex) when (LiveEngineBackendTestGuards.IsLiveEngineUnavailable(ex, "openvoice"))
      {
        Assert.Fail(
          "Live OpenVoice engine not initialized or unavailable; run /api/health/preflight with openvoice.ok before this proof: "
          + ex.Message);
      ...
      Assert.AreEqual(
        ...
        "Backend must echo routed_engine=openvoice (no silent engine substitution).");
      ...
      var outPath = Path.Combine(
        ...
        "slice19",
        "openvoice",
        "openvoice_csharp_stream.wav");
      Directory.CreateDirectory(Path.GetDirectoryName(outPath)!);
      await File.WriteAllBytesAsync(outPath, bytes, CancellationToken.None).ConfigureAwait(false);
      Assert.IsTrue(File.Exists(outPath) && new FileInfo(outPath).Length == bytes.Length, "Failed to write slice19/openvoice/openvoice_csharp_stream.wav.");
    }
    ...
    public async Task Synthesize_ThenPlayback_LiveBackend_PlayableNonSilentWav()
    {
      ...
      {
        profile = await profilesClient.CreateProfileAsync(
          "csharp-slice19-openvoice-playback-audition",
          language: "en",
          cancellationToken: CancellationToken.None).ConfigureAwait(false);
      }
      catch (Exception ex)
      ...
      using (var consentHttp = new HttpClient { BaseAddress = new Uri(BackendBase), Timeout = TimeSpan.FromMinutes(2) })
      {
        var consentReq = JsonSerializer.Serialize(
          new Dictionary<string, string>
          {
            ["voice_id"] = profile.Id,
            ["grantor_id"] = "local",
            ["grantor_name"] = "csharp-slice19-openvoice-playback",
            ["consent_type"] = "voice_usage",
          });
        using var reqContent = new StringContent(
          consentReq,
      ...
      var tempPath = Path.Combine(Path.GetTempPath(), $"vs_slice19_openvoice_{Guid.NewGuid():N}.wav");
