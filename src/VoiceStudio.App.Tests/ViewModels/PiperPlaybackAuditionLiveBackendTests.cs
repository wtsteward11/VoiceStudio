...
{
  /// <summary>
  /// Slice 10 Piper — Live-backend proof: Piper synthesis → <see cref="IVoiceSynthesisService.GetAudioStreamAsync"/> →
  ...
  /// <see cref="RealSynthesisPiperLiveBackendTests"/> (<c>VOICESTUDIO_REAL_XTTS_HTTP_BASE</c>).
  /// </summary>
  [TestClass]
  [TestCategory("LiveBackend")]
  public sealed class PiperPlaybackAuditionLiveBackendTests
  {
    ...
    /// <summary>
    /// Slice 10 Piper stream proof: Piper → <see cref="IVoiceSynthesisService.GetAudioStreamAsync"/> → non-silent PCM16 WAV
    /// (RIFF/fmt/data, duration, peak) and writes <c>docs/reports/verification/slice10/piper/piper_csharp_stream.wav</c>.
    /// No audio output device required. On Piper unavailable, <see cref="Assert.Fail"/> (preflight should gate).
    /// </summary>
    [TestMethod]
    public async Task Synthesize_PrimaryFileRoute_LiveBackend_StreamPlayable()
    {
      ...
      {
        Assert.Inconclusive(
          "Set VOICESTUDIO_TEST_MODE unset (not stub) on the backend process for real Piper proof.");
      }
      ...
      try
      {
        profile = await profilesClient.CreateProfileAsync(
          "csharp-slice10-piper-stream-playable",
          language: "en",
      ...
      using (var consentHttp = new HttpClient { BaseAddress = new Uri(BackendBase), Timeout = TimeSpan.FromMinutes(2) })
      {
        var consentReq = JsonSerializer.Serialize(
          new Dictionary<string, string>
          {
            ["voice_id"] = profile.Id,
            ["grantor_id"] = "local",
            ["grantor_name"] = "csharp-slice10-piper-stream",
      ...
      try
      {
        response = await synthService.SynthesizeVoiceAsync(
          new VoiceSynthesisRequest
          {
            ...
            Engine = "piper",
            Text = "VoiceStudio slice ten piper playback artifact audition proof.",
            Language = "en",
          },
      ...
      catch (BackendException ex) when (LiveEngineBackendTestGuards.IsLiveEngineUnavailable(ex, "piper"))
      {
        Assert.Fail(
          "Live Piper engine not initialized or unavailable; run /api/health/preflight with piper.ok before this proof: "
          + ex.Message);
      ...
      Assert.AreEqual(
        ...
        "Backend must echo routed_engine=piper (no silent engine substitution).");
      ...
      var outPath = Path.Combine(
        ...
        "slice10",
        "piper",
        "piper_csharp_stream.wav");
      Directory.CreateDirectory(Path.GetDirectoryName(outPath)!);
      await File.WriteAllBytesAsync(outPath, bytes, CancellationToken.None).ConfigureAwait(false);
      Assert.IsTrue(File.Exists(outPath) && new FileInfo(outPath).Length == bytes.Length, "Failed to write slice10/piper_csharp_stream.wav.");
    }
    ...
    public async Task Synthesize_ThenPlayback_LiveBackend_PlayableNonSilentWav()
    {
      ...
      {
        profile = await profilesClient.CreateProfileAsync(
          "csharp-slice10-piper-playback-audition",
          language: "en",
          cancellationToken: CancellationToken.None).ConfigureAwait(false);
      }
      catch (Exception ex)
      ...
      using (var consentHttp = new HttpClient { BaseAddress = new Uri(BackendBase), Timeout = TimeSpan.FromMinutes(2) })
      {
        var consentReq = JsonSerializer.Serialize(
          new Dictionary<string, string>
          {
            ["voice_id"] = profile.Id,
            ["grantor_id"] = "local",
            ["grantor_name"] = "csharp-slice10-piper-playback",
            ["consent_type"] = "voice_usage",
          });
        using var reqContent = new StringContent(
          consentReq,
      ...
      var tempPath = Path.Combine(Path.GetTempPath(), $"vs_slice10_piper_{Guid.NewGuid():N}.wav");
