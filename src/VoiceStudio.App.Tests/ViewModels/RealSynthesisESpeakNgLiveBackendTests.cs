...
{
  /// <summary>
  /// Live-backend proof: real <c>espeak_ng</c> synthesis (non-stub) through
  ...
  public sealed class RealSynthesisESpeakNgLiveBackendTests
  {
    ...
    [TestMethod]
    public async Task Synthesize_ESpeakNg_LiveBackend_ServiceReturnsAudio_NonSilentWav()
    {
      ...
      using (var consentHttp = new HttpClient { BaseAddress = new Uri(BackendBase), Timeout = TimeSpan.FromMinutes(2) })
      {
        var consentReq = JsonSerializer.Serialize(
          new Dictionary<string, string>
          {
            ["voice_id"] = profile.Id,
            ["grantor_id"] = "local",
            ["grantor_name"] = "csharp-slice12-espeak-ng-real",
      ...
      try
      {
        response = await synthService.SynthesizeVoiceAsync(
          new VoiceSynthesisRequest
          {
            ...
            Engine = "espeak_ng",
            Text = "VoiceStudio slice twelve espeak ng real synthesis.",
            Language = "en",
          },
      ...
      {
        Assert.Inconclusive(
          "Live eSpeak NG engine not initialized or unavailable (same posture as pytest skip when engines_ready / model missing): "
          + ex.Message);
        return;
      }
