...
{
  /// <summary>
  /// Live-backend proof: real <c>openvoice</c> synthesis (non-stub) through
  ...
  /// Override base URL with environment variable <c>VOICESTUDIO_REAL_XTTS_HTTP_BASE</c> (same as other real_* proofs).
  /// </summary>
  [TestClass]
  [TestCategory("LiveBackend")]
  public sealed class RealSynthesisOpenVoiceLiveBackendTests
  {
    ...
    [TestMethod]
    public async Task Synthesize_OpenVoice_LiveBackend_ServiceReturnsAudio_NonSilentWav()
    {
      ...
      using (var consentHttp = new HttpClient { BaseAddress = new Uri(BackendBase), Timeout = TimeSpan.FromMinutes(2) })
      {
        var consentReq = JsonSerializer.Serialize(
          new Dictionary<string, string>
          {
            ["voice_id"] = profile.Id,
            ["grantor_id"] = "local",
            ["grantor_name"] = "csharp-slice19-openvoice-real",
      ...
      try
      {
        response = await synthService.SynthesizeVoiceAsync(
          new VoiceSynthesisRequest
          {
            ...
            Engine = "openvoice",
            Text = "VoiceStudio slice nineteen openvoice real synthesis.",
            Language = "en",
          },
      ...
      {
        Assert.Inconclusive(
          "Live OpenVoice engine not initialized or unavailable (same posture as pytest skip when engines_ready / model missing): "
          + ex.Message);
        return;
      }
