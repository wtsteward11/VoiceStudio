...
{
  /// <summary>
  /// Live-backend proof: real <c>piper</c> synthesis (non-stub) through
  ...
  public sealed class RealSynthesisPiperLiveBackendTests
  {
    ...
    public async Task Synthesize_XttsV2_LiveBackend_ServiceReturnsAudio_NonSilentWav()
    {
      ...
      using (var consentHttp = new HttpClient { BaseAddress = new Uri(BackendBase), Timeout = TimeSpan.FromMinutes(2) })
      {
        var consentReq = JsonSerializer.Serialize(
          new Dictionary<string, string>
          {
            ["voice_id"] = profile.Id,
            ["grantor_id"] = "local",
            ["grantor_name"] = "csharp-slice10-piper-real",
      ...
      try
      {
        response = await synthService.SynthesizeVoiceAsync(
          new VoiceSynthesisRequest
          {
            ...
            Engine = "piper",
            Text = "VoiceStudio slice ten piper real synthesis.",
            Language = "en",
          },
      ...
      {
        Assert.Inconclusive(
          "Live Piper engine not initialized or unavailable (same posture as pytest skip when engines_ready / model missing): "
          + ex.Message);
        return;
      }
