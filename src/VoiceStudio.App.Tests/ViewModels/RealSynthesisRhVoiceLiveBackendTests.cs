...
{
  /// <summary>
  /// Live-backend proof: real <c>rhvoice</c> synthesis (non-stub) through
  ...
  /// Override base URL with environment variable <c>VOICESTUDIO_REAL_XTTS_HTTP_BASE</c> (same as Python real_rhvoice test).
  /// </summary>
  [TestClass]
  [TestCategory("LiveBackend")]
  public sealed class RealSynthesisRhVoiceLiveBackendTests
  {
    ...
    [TestMethod]
    public async Task Synthesize_RhVoice_LiveBackend_ServiceReturnsAudio_NonSilentWav()
    {
      ...
      using (var consentHttp = new HttpClient { BaseAddress = new Uri(BackendBase), Timeout = TimeSpan.FromMinutes(2) })
      {
        var consentReq = JsonSerializer.Serialize(
          new Dictionary<string, string>
          {
            ["voice_id"] = profile.Id,
            ["grantor_id"] = "local",
            ["grantor_name"] = "csharp-slice14-rhvoice-real",
      ...
      try
      {
        response = await synthService.SynthesizeVoiceAsync(
          new VoiceSynthesisRequest
          {
            ...
            Engine = "rhvoice",
            Text = "VoiceStudio slice fourteen rhvoice real synthesis.",
            Language = "en",
          },
      ...
      {
        Assert.Inconclusive(
          "Live RHVoice engine not initialized or unavailable (same posture as pytest skip when engines_ready / binary missing): "
          + ex.Message);
        return;
      }
