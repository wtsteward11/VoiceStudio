...
{
  /// <summary>
  /// Live-backend proof: real <c>silero</c> synthesis (non-stub) through
  ...
  public sealed class RealSynthesisSileroLiveBackendTests
  {
    ...
    [TestMethod]
    public async Task Synthesize_Silero_LiveBackend_ServiceReturnsAudio_NonSilentWav()
    {
      ...
      using (var consentHttp = new HttpClient { BaseAddress = new Uri(BackendBase), Timeout = TimeSpan.FromMinutes(2) })
      {
        var consentReq = JsonSerializer.Serialize(
          new Dictionary<string, string>
          {
            ["voice_id"] = profile.Id,
            ["grantor_id"] = "local",
            ["grantor_name"] = "csharp-slice15-silero-real",
      ...
      try
      {
        response = await synthService.SynthesizeVoiceAsync(
          new VoiceSynthesisRequest
          {
            ...
            Engine = "silero",
            Text = "VoiceStudio slice fifteen silero real synthesis.",
            Language = "en",
          },
      ...
      {
        Assert.Inconclusive(
          "Live Silero TTS engine not initialized or unavailable (same posture as pytest skip when engines_ready / model missing): "
          + ex.Message);
        return;
      }
