...
{
  /// <summary>
  /// Live-backend proof: real <c>tortoise</c> synthesis (non-stub) through
  ...
  public sealed class RealSynthesisTortoiseLiveBackendTests
  {
    ...
    [TestMethod]
    public async Task Synthesize_Tortoise_LiveBackend_ServiceReturnsAudio_NonSilentWav()
    {
      ...
      {
        Assert.Inconclusive(
          "Set VOICESTUDIO_TEST_MODE unset (not stub) on the backend process for real Tortoise proof.");
      }
      ...
      using (var consentHttp = new HttpClient { BaseAddress = new Uri(BackendBase), Timeout = TimeSpan.FromMinutes(2) })
      {
        var consentReq = JsonSerializer.Serialize(
          new Dictionary<string, string>
          {
            ["voice_id"] = profile.Id,
            ["grantor_id"] = "local",
            ["grantor_name"] = "csharp-slice18-tortoise-real",
      ...
      try
      {
        response = await synthService.SynthesizeVoiceAsync(
          new VoiceSynthesisRequest
          {
            ...
            Engine = "tortoise",
            Text = "VoiceStudio slice eighteen tortoise real synthesis.",
            Language = "en",
          },
      ...
      {
        Assert.Inconclusive(
          "Live Tortoise TTS engine not initialized or unavailable (same posture as pytest skip when engines_ready / model missing): "
          + ex.Message);
        return;
      }
