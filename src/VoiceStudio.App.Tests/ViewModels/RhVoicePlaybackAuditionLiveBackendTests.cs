...
{
  /// <summary>
  /// Slice 14 RHVoice — Live-backend proof: RHVoice synthesis → <see cref="IVoiceSynthesisService.GetAudioStreamAsync"/> →
  ...
  /// <see cref="RealSynthesisRhVoiceLiveBackendTests"/> (<c>VOICESTUDIO_REAL_XTTS_HTTP_BASE</c>).
  /// </summary>
  [TestClass]
  [TestCategory("LiveBackend")]
  public sealed class RhVoicePlaybackAuditionLiveBackendTests
  {
    ...
    /// <summary>
    /// Slice 14 RHVoice stream proof: RHVoice → <see cref="IVoiceSynthesisService.GetAudioStreamAsync"/> → non-silent PCM16 WAV
    /// (RIFF/fmt/data, duration, peak) and writes <c>docs/reports/verification/slice14/rhvoice/rhvoice_csharp_stream.wav</c>.
    /// No audio output device required. On RHVoice unavailable, <see cref="Assert.Fail"/> (preflight should gate).
    /// </summary>
    [TestMethod]
    public async Task Synthesize_PrimaryFileRoute_LiveBackend_StreamPlayable()
    {
      ...
      {
        Assert.Inconclusive(
          "Set VOICESTUDIO_TEST_MODE unset (not stub) on the backend process for real RHVoice proof.");
      }
      ...
      await LivePreflightGuards.AssertRhVoicePreflightOkAsync(probe, BackendBase, CancellationToken.None)
      ...
      try
      {
        profile = await profilesClient.CreateProfileAsync(
          "csharp-slice14-rhvoice-stream-playable",
          language: "en",
      ...
      using (var consentHttp = new HttpClient { BaseAddress = new Uri(BackendBase), Timeout = TimeSpan.FromMinutes(2) })
      {
        var consentReq = JsonSerializer.Serialize(
          new Dictionary<string, string>
          {
            ["voice_id"] = profile.Id,
            ["grantor_id"] = "local",
            ["grantor_name"] = "csharp-slice14-rhvoice-stream",
      ...
      try
      {
        response = await synthService.SynthesizeVoiceAsync(
          new VoiceSynthesisRequest
          {
            ...
            Engine = "rhvoice",
            Text = "VoiceStudio slice fourteen rhvoice playback artifact audition proof.",
            Language = "en",
          },
      ...
      catch (BackendException ex) when (ex.StatusCode == 400)
      {
        ...
        if (m.Contains("Invalid engine", StringComparison.OrdinalIgnoreCase)
            && m.Contains("rhvoice", StringComparison.OrdinalIgnoreCase))
        {
          Assert.Fail(
            "Synthesis returned 400 Invalid engine after checks.rhvoice preflight ok; "
      ...
      catch (BackendException ex) when (LiveEngineBackendTestGuards.IsLiveEngineUnavailable(ex, "rhvoice"))
      {
        Assert.Fail(
          "Live RHVoice engine not initialized or unavailable; run /api/health/preflight with rhvoice.ok before this proof: "
          + ex.Message);
      ...
      Assert.AreEqual(
        ...
        "Backend must echo routed_engine=rhvoice (no silent engine substitution).");
      ...
      var outPath = Path.Combine(
        ...
        "slice14",
        "rhvoice",
        "rhvoice_csharp_stream.wav");
      Directory.CreateDirectory(Path.GetDirectoryName(outPath)!);
      await File.WriteAllBytesAsync(outPath, bytes, CancellationToken.None).ConfigureAwait(false);
      Assert.IsTrue(File.Exists(outPath) && new FileInfo(outPath).Length == bytes.Length, "Failed to write slice14/rhvoice_csharp_stream.wav.");
    }
    ...
    public async Task Synthesize_ThenPlayback_LiveBackend_PlayableNonSilentWav()
    {
      ...
      {
        profile = await profilesClient.CreateProfileAsync(
          "csharp-slice14-rhvoice-playback-audition",
          language: "en",
          cancellationToken: CancellationToken.None).ConfigureAwait(false);
      }
      catch (Exception ex)
      ...
      using (var consentHttp = new HttpClient { BaseAddress = new Uri(BackendBase), Timeout = TimeSpan.FromMinutes(2) })
      {
        var consentReq = JsonSerializer.Serialize(
          new Dictionary<string, string>
          {
            ["voice_id"] = profile.Id,
            ["grantor_id"] = "local",
            ["grantor_name"] = "csharp-slice14-rhvoice-playback",
            ["consent_type"] = "voice_usage",
          });
        using var reqContent = new StringContent(
          consentReq,
      ...
      var tempPath = Path.Combine(Path.GetTempPath(), $"vs_slice14_rhvoice_{Guid.NewGuid():N}.wav");
