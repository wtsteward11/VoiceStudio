...
{
  /// <summary>
  /// Slice 15 Silero TTS — Live-backend proof: Silero TTS synthesis → <see cref="IVoiceSynthesisService.GetAudioStreamAsync"/> →
  ...
  /// <see cref="RealSynthesisSileroLiveBackendTests"/> (<c>VOICESTUDIO_REAL_XTTS_HTTP_BASE</c>).
  /// </summary>
  [TestClass]
  [TestCategory("LiveBackend")]
  public sealed class SileroPlaybackAuditionLiveBackendTests
  {
    ...
    /// <summary>
    /// Slice 15 Silero TTS stream proof: Silero TTS → <see cref="IVoiceSynthesisService.GetAudioStreamAsync"/> → non-silent PCM16 WAV
    /// (RIFF/fmt/data, duration, peak) and writes <c>docs/reports/verification/slice15/silero/silero_csharp_stream.wav</c>.
    /// No audio output device required. On Silero TTS unavailable, <see cref="Assert.Fail"/> (preflight should gate).
    /// </summary>
    [TestMethod]
    public async Task Synthesize_PrimaryFileRoute_LiveBackend_StreamPlayable()
    {
      ...
      {
        Assert.Inconclusive(
          "Set VOICESTUDIO_TEST_MODE unset (not stub) on the backend process for real Silero TTS proof.");
      }
      ...
      await LivePreflightGuards.AssertSileroPreflightOkAsync(probe, BackendBase, CancellationToken.None)
      ...
      try
      {
        profile = await profilesClient.CreateProfileAsync(
          "csharp-slice15-silero-stream-playable",
          language: "en",
      ...
      using (var consentHttp = new HttpClient { BaseAddress = new Uri(BackendBase), Timeout = TimeSpan.FromMinutes(2) })
      {
        var consentReq = JsonSerializer.Serialize(
          new Dictionary<string, string>
          {
            ["voice_id"] = profile.Id,
            ["grantor_id"] = "local",
            ["grantor_name"] = "csharp-slice15-silero-stream",
      ...
      try
      {
        response = await synthService.SynthesizeVoiceAsync(
          new VoiceSynthesisRequest
          {
            ...
            Engine = "silero",
            Text = "VoiceStudio slice fifteen silero playback artifact audition proof.",
            Language = "en",
          },
      ...
      catch (BackendException ex) when (ex.StatusCode == 400)
      {
        ...
        if (m.Contains("Invalid engine", StringComparison.OrdinalIgnoreCase)
            && m.Contains("silero", StringComparison.OrdinalIgnoreCase))
        {
          Assert.Fail(
            "Synthesis returned 400 Invalid engine after checks.silero preflight ok; "
      ...
      catch (BackendException ex) when (LiveEngineBackendTestGuards.IsLiveEngineUnavailable(ex, "silero"))
      {
        Assert.Fail(
          "Live Silero TTS engine not initialized or unavailable; run /api/health/preflight with silero.ok before this proof: "
          + ex.Message);
      ...
      Assert.AreEqual(
        ...
        "Backend must echo routed_engine=silero (no silent engine substitution).");
      ...
      var outPath = Path.Combine(
        ...
        "slice15",
        "silero",
        "silero_csharp_stream.wav");
      Directory.CreateDirectory(Path.GetDirectoryName(outPath)!);
      await File.WriteAllBytesAsync(outPath, bytes, CancellationToken.None).ConfigureAwait(false);
      Assert.IsTrue(File.Exists(outPath) && new FileInfo(outPath).Length == bytes.Length, "Failed to write slice15/silero_csharp_stream.wav.");
    }
    ...
    public async Task Synthesize_ThenPlayback_LiveBackend_PlayableNonSilentWav()
    {
      ...
      {
        profile = await profilesClient.CreateProfileAsync(
          "csharp-slice15-silero-playback-audition",
          language: "en",
          cancellationToken: CancellationToken.None).ConfigureAwait(false);
      }
      catch (Exception ex)
      ...
      using (var consentHttp = new HttpClient { BaseAddress = new Uri(BackendBase), Timeout = TimeSpan.FromMinutes(2) })
      {
        var consentReq = JsonSerializer.Serialize(
          new Dictionary<string, string>
          {
            ["voice_id"] = profile.Id,
            ["grantor_id"] = "local",
            ["grantor_name"] = "csharp-slice15-silero-playback",
            ["consent_type"] = "voice_usage",
          });
        using var reqContent = new StringContent(
          consentReq,
      ...
      var tempPath = Path.Combine(Path.GetTempPath(), $"vs_slice15_silero_{Guid.NewGuid():N}.wav");
