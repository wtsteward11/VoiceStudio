"""
Real Chatterbox TTS synthesis against a **live** FastAPI backend (non-stub).

Same HTTP shape as ``test_synthesis_silero_real.py`` but with ``engine=chatterbox`` and
``GET /api/health/preflight`` gate on ``checks.chatterbox.ok``.
...
async def _preflight_chatterbox_ok(client: AsyncClient) -> bool:
    try:
    ...
    cb = checks.get("chatterbox")
    if not isinstance(cb, dict):
        return False
    return cb.get("ok") is True


def _skip_if_chatterbox_engine_unavailable(synth_resp: Response) -> None:
    if synth_resp.status_code not in (500, 503):
        return
    body = synth_resp.text.lower()
    if "chatterbox" in body and (
        ...
        pytest.skip(
            "Live Chatterbox engine not initialized or unavailable (opt-in real_chatterbox proof)."
        )


async def _synthesize_chatterbox_and_fetch_primary_file(
    client: AsyncClient,
    *,
    profile_name: str,
    ...
    _skip_if_chatterbox_engine_unavailable(synth_resp)
    ...
    assert synth_data.get("routed_engine") == "chatterbox", (
        f"Expected routed_engine=chatterbox, got {synth_data.get('routed_engine')!r}"
    )
...
@pytest.fixture
async def live_chatterbox_client():
    """HTTP client; gates on preflight chatterbox.ok == True."""
    ...
    async with AsyncClient(base_url=base, timeout=900.0) as client:
        try:
            ...
            pytest.skip(
                f"Real Chatterbox proof requires live backend at {base}. Not reachable: {exc}"
            )
        ...
        if not await _preflight_chatterbox_ok(client):
            pytest.skip(
                f"Preflight checks.chatterbox.ok is not true at {base}/api/health/preflight; "
                "install chatterbox-tts deps + warm HF cache ResembleAI/chatterbox (see ensure_chatterbox)."
            )
        yield client
...
@pytest.mark.slow
@pytest.mark.real_chatterbox
@pytest.mark.timeout(900)
async def test_real_chatterbox_synthesize_returns_audible_wav(
    live_chatterbox_client: AsyncClient,
) -> None:
    if _stub_like_mode():
        pytest.skip("VOICESTUDIO_TEST_MODE is stub-like; real-synthesis proof requires it unset.")

    profile_id, audio_id, audio_resp = await _synthesize_chatterbox_and_fetch_primary_file(
        live_chatterbox_client,
        profile_name="slice17-chatterbox-real",
        profile_description="Slice 17 real Chatterbox live HTTP proof",
        synth_text="VoiceStudio slice seventeen chatterbox real synthesis proof.",
    )
    ...
    proof_wav = proof_dir / "chatterbox_output.wav"
    proof_wav.write_bytes(raw)
    snippet = proof_dir / "chatterbox_backend_log_snippet.txt"
    snippet.write_text(
        "Slice 17 real Chatterbox live-backend proof (synthesis succeeded).\n"
...
@pytest.mark.real_chatterbox
@pytest.mark.timeout(900)
async def test_real_chatterbox_primary_audio_file_route_content_type(
    live_chatterbox_client: AsyncClient,
) -> None:
    ...
    _profile_id, audio_id, audio_resp = await _synthesize_chatterbox_and_fetch_primary_file(
        live_chatterbox_client,
        profile_name="slice17-chatterbox-file-route",
        profile_description="Slice 17 Chatterbox file route proof",
        synth_text="VoiceStudio slice seventeen chatterbox file route proof.",
    )
    assert audio_resp.status_code == 200
