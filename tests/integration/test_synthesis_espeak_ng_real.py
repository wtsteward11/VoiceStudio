"""
Real eSpeak NG TTS synthesis against a **live** FastAPI backend (non-stub).

Same HTTP shape as ``test_synthesis_xtts_real.py`` but with ``engine=espeak_ng`` and
``GET /api/health/preflight`` gate on ``checks.espeak_ng.ok``.

Override base URL: ``VOICESTUDIO_REAL_XTTS_HTTP_BASE`` (shared with XTTS live proofs).

Run explicitly:
  python -m pytest tests/integration/test_synthesis_espeak_ng_real.py -v -m real_espeak_ng --tb=short
"""
...
async def _preflight_espeak_ng_ok(client: AsyncClient) -> bool:
    try:
    ...
    es = checks.get("espeak_ng")
    if not isinstance(es, dict):
        return False
    return es.get("ok") is True


def _skip_if_espeak_ng_engine_unavailable(synth_resp: Response) -> None:
    if synth_resp.status_code not in (500, 503):
        return
    body = synth_resp.text.lower()
    if "espeak" in body and (
        "not available" in body
        or "failed to initialize" in body
        or "503" in body
    ):
        pytest.skip(
            "Live eSpeak NG engine not initialized or unavailable (opt-in real_espeak_ng proof)."
        )


async def _synthesize_espeak_ng_and_fetch_primary_file(
    client: AsyncClient,
    *,
    profile_name: str,
    ...
    _skip_if_espeak_ng_engine_unavailable(synth_resp)
    ...
    assert synth_data.get("routed_engine") == "espeak_ng", (
        f"Expected routed_engine=espeak_ng, got {synth_data.get('routed_engine')!r}"
    )
...
@pytest.fixture
async def live_espeak_ng_client():
    """HTTP client; gates on preflight espeak_ng.ok == True."""
    ...
    async with AsyncClient(base_url=base, timeout=900.0) as client:
        try:
            ...
            pytest.skip(
                f"Real eSpeak NG proof requires live backend at {base}. Not reachable: {exc}"
            )
        ...
        if not await _preflight_espeak_ng_ok(client):
            pytest.skip(
                f"Preflight checks.espeak_ng.ok is not true at {base}/api/health/preflight; install eSpeak NG CLI."
            )
        yield client
...
@pytest.mark.slow
@pytest.mark.real_espeak_ng
@pytest.mark.timeout(900)
async def test_real_espeak_ng_synthesize_returns_audible_wav(
    live_espeak_ng_client: AsyncClient,
) -> None:
    if _stub_like_mode():
        pytest.skip("VOICESTUDIO_TEST_MODE is stub-like; real-synthesis proof requires it unset.")

    profile_id, audio_id, audio_resp = await _synthesize_espeak_ng_and_fetch_primary_file(
        live_espeak_ng_client,
        profile_name="slice12-espeak-ng-real",
        profile_description="Slice 12 real eSpeak NG live HTTP proof",
        synth_text="VoiceStudio slice twelve espeak ng real synthesis proof.",
    )
    ...
    proof_wav = proof_dir / "espeak_ng_output.wav"
    proof_wav.write_bytes(raw)
    snippet = proof_dir / "espeak_ng_backend_log_snippet.txt"
    snippet.write_text(
        "Slice 12 real eSpeak NG live-backend proof (synthesis succeeded).\n"
...
@pytest.mark.real_espeak_ng
@pytest.mark.timeout(900)
async def test_real_espeak_ng_primary_audio_file_route_content_type(
    live_espeak_ng_client: AsyncClient,
) -> None:
    ...
    _profile_id, audio_id, audio_resp = await _synthesize_espeak_ng_and_fetch_primary_file(
        live_espeak_ng_client,
        profile_name="slice12-espeak-ng-file-route",
        profile_description="Slice 12 eSpeak NG file route proof",
        synth_text="VoiceStudio slice twelve espeak ng file route proof.",
    )
    assert audio_resp.status_code == 200
